...
public class PlacebleItem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, Vector3.down, out groundHit, boxHeight * 0.5f, LayerMask.GetMask("Ground")))//can only be placed if half of the box is on the ground 
        {
            isGrounded = true;
        }
        else
